...
class ViewController: UIViewController, AVAssetResourceLoaderDelegate {
    ...
    private func setupDeepARAndCamera() {
        ...
        self.deepAR.setLicenseKey("Put key here")
...
extension ViewController: DeepARDelegate {
    ...
    func didFinishVideoRecording(_ videoFilePath: String!) {
        ...
        do {
            ...
            saveVideoToCameraRoll(videoUrl: destination)
    ...
    func saveVideoToCameraRoll(videoUrl: URL) {
        PHPhotoLibrary.requestAuthorization { (aut) in
            if aut == .authorized {
    ...
    private func saveVideo(videoURL: URL?) {
        ...
        let fileName =  URL(fileURLWithPath: videoURL?.path ?? "").lastPathComponent.replacingOccurrences(of: ".mov", with: "")
        
        guard FileManager.getFile(fileName: fileName + ".mov", folder: .recorded) != nil else {
            ...
            guard let outputURL = FileManager.createPathFor(location: .recorded,
                                                            fileName: fileName) else { return }
            self.saveVideoToCameraRoll(videoUrl: outputURL)
//            exporter?.outputURL = outputURL
//            exporter?.outputFileType = AVFileType.mov
//
//            exporter?.exportAsynchronously(completionHandler: {
//
//                print("status: \((exporter?.status.rawValue))")
//                print("error: \(exporter?.error)")
//                print("exporter?.outputURL: \(exporter?.outputURL)")
//
//                self.saveVideoToCameraRoll(videoUrl: (exporter?.outputURL)!)
//
//            })
            return
        }
